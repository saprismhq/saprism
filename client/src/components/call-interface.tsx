...
export function CallInterface({ meeting, isLoading, onSessionUpdate }: CallInterfaceProps) {
  ...
  useEffect(() => {
    ...
    callService.onConnectionStateChanged((state) => {
      ...
      if (currentSession) {
        sessionService.updateConnectionState(currentSession.id, state);
      }
    });
    ...
    <Card className="h-full flex flex-col max-h-full">
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-medium text-gray-900 flex items-center">
          ...
          Call with {meeting.clientName}
          {meeting.clientCompany && <span className="text-gray-500 ml-1">at {meeting.clientCompany}</span>}
        </CardTitle>
      </CardHeader>
              ...
              <div className="text-center text-white">
                {isConnecting ? (
                  ...
                  <>
                    ...
                    <p className="text-gray-400">Ready to start call</p>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="border-t pt-3">
            ...
            <div className="space-y-1">
              {participants.map((participant) => (
                <div key={participant.sid} className="flex items-center justify-between text-sm">
